...
$req = "SELECT id,poste,compteur_like,compteur_retweet,num_tweet,retweeted,retweeted_by,commented,commented_by,compteur_comments FROM actualite ORDER BY num_tweet DESC limit 100";
			...
			$res = $req->fetch();
				...
				while($res = $req->fetch()){
				if($res['id']==$data1['id']){
				$req3="select Prenom,Nom,imageprofil from profil2 where id=:id"; //personne qui a tweeter
				$req3=$bdd->prepare($req3);
				$req3->execute(array('id'=>$res['id']));
				$data3=$req3->fetch();
				
				$req4=$bdd->prepare("select Prenom,Nom,imageprofil from profil2 where id=:retweeted_by"); //personne qui a retweeter
				$req4->execute(array('retweeted_by'=>$res['retweeted_by']));
				$data4=$req4->fetch();
				
				
			$req5=$bdd->prepare("select Prenom, Nom, imageprofil from profil2 where id=:commented_by");  //personne qui a commenté
					...
					<div id="contenu_droit">
						<p id="nom_profil">'.$data3['Prenom']." ".$data3['Nom'].'</p>
							...
							<div class="image_like">	
								...
								<a href="retweet_traitement?num_tweet='.$res['num_tweet'].'"'.'><img src="retweet.png" class="image2" /></a>
								<p class="retweet" >'.$res['compteur_retweet'].'</p>
								<img src="message.png" class="image3" />
								<p class="comment" >'.$res['compteur_comments'].'</p>
							</div>
								<div class="commentaire_conteneur">
								<img src="'.$data5['imageprofil'].'" />
								...
								<form method="POST" action="message_traitement.php?num_tweet='.$res['num_tweet'].'" id="form_comment">	
									<img src="'.$_SESSION['imageprofil'].'" />
